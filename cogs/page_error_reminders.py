...
class PageErrorReminders(commands.Cog):
    ...
    async def check_wiki_page_errors(self, channel: discord.TextChannel) -> None:
        ...
        data = await self.fetch_categories("darkbarbarian.fandom.com", "Benutzer:DarkBarbarian/WikiCategories.json")
        if data:
        ...
        for wiki, categories in self.wiki_categories.items():
            ...
            for category in categories:
                ...
                pages = await self.fetch_category_members(wiki, category)
    ...
    async def fetch_categories(self, wiki: str, page_title: str) -> dict | None:
        ...
        try:
            response = await requests_async.get(url, params=params, timeout=30)
